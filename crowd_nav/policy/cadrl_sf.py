...
def mlp(input_dim, mlp_dims, last_relu=False):
    ...
    layers.append(nn.Linear(mlp_dims[-1], input_dim))
...
class ValueNetwork(nn.Module):
    def __init__(self, input_dim, mlp_dims):
        ...
        self.w_vec = torch.tensor(np.random.randn(input_dim).reshape(1, -1)).float()
...
class CADRL_SF(Policy):
    ...
    def configure(self, config):
        ...
        mlp_dims = [int(x) for x in config.get('cadrl_sf', 'mlp_dims').split(', ')]
        self.model = ValueNetwork(self.joint_state_dim, mlp_dims)
        self.multiagent_training = config.getboolean('cadrl_sf', 'multiagent_training')
        logging.info('Policy: CADRL_SF without occupancy map')
    ...
    def predict(self, state):
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            for action in self.action_space:
                ...
                # VALUE UPDATE
                outputs = self.model(self.rotate(batch_next_states)) # Successor feature
